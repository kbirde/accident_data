"""This module is for plotting US statewise accident location map & a Donut chart
for accidents occurring on different days of the week """
import argparse
import pandas as pd
...
def map_donut(accident_file , input_state, year):
    ...
    try:
        ...
        # Reading in 2010 ACCIDENT data, downselecting the rows related to the user's state
        # choice & reducing the dataframe to 4 columns
        ...
        # Setting up a subplot with 1 row and 2 columns, specifying that the first column
        # is a scattergeo map and the second column is a pie chart
        fig = make_subplots(rows=1, cols=2,
                            column_widths=[0.6,0.4],
                            row_heights=[0.4],
                            specs=[[{"type": "scattergeo"}, {"type": "pie"}]])

    # Adding a Scattergeo trace with latitude, longitude, marker (w/ color), title &
    # focusing map to USA region
        fig.add_trace(
    ...
    # Updating figure layout with different titles
    # NOTE: the main title dynamically updates the
    # state name as selected by the user
        fig.update_layout(
            geo_scope='usa',
            height = 500,
            width = 900,
            title={
                'text': f"<b> Accident Analysis for {get_state} ({year})",
                'y':0.9,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'},
...
if __name__ == '__main__':
    ...
    map_donut(args.accident_file, args.input_state, args.year)
